...
export default function Feed({ navigation }) {
  ...
  const getPosts = () => {
    axios.get('http://192.168.0.29:5000/posts/get-posts')
      .then((response) => {
  ...
  const fetchAuthors = async () => {
    ...
    for (const post of posts) {
      try {
        const response = await axios.get("http://192.168.0.29:5000/users/" + post.author);
        authorDetails[post.author] = response.data;
      } catch (error) {
    ...
    <View style={{ flex: 1, display: "flex", alignItems: "center"}}>
      {posts.map((post, index) => (
        <View key={index} style={{width: "100%", alignItems: "center"}}>
          <TouchableOpacity onPress={() => navigation.navigate("Post")} style={{width: "90%"}} key={post.id}>
            <View style={styles.card}>
                ...
                <View style={{display: "flex", flexDirection: "row"}}>
                  ...
                  <View style={{display: "flex"}}>
                    <View style={{display: "flex", flexDirection: "row", justifyContent: "center"}}>
                      <Text style={{fontFamily: "Roboto-Bold", marginHorizontal:6, fontSize:16}}>{authors[post.author] ? authors[post.author]['user']['username'] : 'Loading...'}</Text>
                      <Text style={{fontFamily: "Roboto-Light", opacity: 0.5}}>@{authors[post.author] ? authors[post.author]['user']['full_name'] : 'Loading...'}</Text>
                    </View>
              ...
              <View style={{display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end"}}>
                <View style={{display: "flex", flexDirection: "row"}}>
                  ...
                  <TouchableOpacity
                    onPress={() => shareContent(post.content + ' - ' + authors[post.author]['user']['username'] + ' @' + authors[post.author]['user']['full_name'] + ' - YMedia')}
                  >
                    <Svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={20}
